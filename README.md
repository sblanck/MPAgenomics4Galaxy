...
How to install MPAgenomics4Galaxy  <a name="how-to-install-mpa" /> [[toc]](#toc)
------------------------
...
Rscript install.R
```
How to use MPAgenomics4Galaxy  <a name="how-to-use-mpa" /> [[toc]](#toc)
------------------------

### Get data  <a name="how-to-use-mpa" /> [[toc]](#toc)
...
### Upload data on Galaxy  <a name="upload" /> [[toc]](#toc)
...
### Preprocess and normalization  <a name="normalization" /> [[toc]](#toc)
...
### Segmentation and Calling of normalized data  <a name="seg-call" /> [[toc]](#toc)
...
![enter image description here](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/segcall2.png)

*The Segmentation and Calling of normalized data* tool have the following inputs :
* A dataset summary file generated by the *Data normalization* tool
* A file selection mode, to choose files to be segmented
	* Either the whole dataset 
	* Or a selection of  few files
* An optionnal tumor-control reference .csv files
* The chromosomes to be processed
* The segmentation method (cghseq or PELT method)
...
* An optionnal .zip files containing the figures of the segmentation
* An optionnal log file

Here is an example of a .scr file :

![example of an scr file](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/scr.png)
...
### Extract  <a name="extract" /> [[toc]](#toc)
This tool extracts the copy number or the allele B fraction profile from the normalized data.

![enter image description here](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/extractbis.png)
...
### Segmentation and Calling of a normalized signal  matrix data <a name="seg-call-extracted" /> [[toc]](#toc)
...
* A .scr (segmentation and calling result) file containing 7 columns :
	...
	*  means: Mean of the segment.
	*  calls: Calling of the segment (”double loss”, ”loss”, ”normal”, ”gain” or ”amplification”).
* An pdf file containing the figures of the segmentation
* An optionnal log file

### Filtering  <a name="filtering" /> [[toc]](#toc)
...
### Markers selection  <a name="markers-selection" /> [[toc]](#toc)
...
![Marker Selection](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/Selection1.png)
![Marker Selection](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/Selection2.png)

The inputs are :
*  A dataset summary file generated by the *Data normalization* tool
* A response .csv file
* The signal you want to work on : copy number (CN) or allele B fraction (fracB)
* An optionnal normal-tumor csv file
* The number of folds for cross validation (must be lower than the number of samples)
* The response type : Linear or Logistic
* The R package used to select relevant markers : HDPenReg or spikslab (for linear reponse only)
...
![Marker Selection](https://github.com/sblanck/MPAgenomics4Galaxy/raw/master/images/selection_results.png)
...
*   Columns are separated by a comma
*   _Extensions of the files (.CEL for example) should be removed_
...
### Markers selection from normalized signal matrix data <a name="markers-selection-extracted" /> [[toc]](#toc)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjMyNzk5Mzc5XX0=
-->